...
hidden_layer_comp = (100,50,)
#Granulatity to round to
round_base = 30#5 #Will be off by round_base/2 at most. Ex. With round_base=20, 11 rounds to 20, 10 rounds to 0
#List of the indices of the columns to use as features
feature_list = [2,4,11,21,31,56]#[11,21,56] #Month(2), Day of Week(4), Listed Departure Time(31)
...
print("Rounding base:", round_base)
...
for num, row in enumerate(csv_data_list[1:400000]):
	...
	try:
		...
		if float(info[label]) > 0: #39 for Departure, 50 or Arrival
			y_temp = int(round_base * round(float(info[label])/round_base)) #Round to nearest specified base
		else:
			y_temp = 0
...
#print(avg_list)
print("----Took", time.time() - start_time, "seconds to load and test", num_iter, "times----")
